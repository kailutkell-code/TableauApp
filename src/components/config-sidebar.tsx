...
export default function ConfigSidebar({ config, onChange }: Props) {
  ...
  const update = (key: keyof LiftConfig, value: unknown) => {
    onChange(prev => {
      ...
      if (key === "bauweise") {
        ...
        if (value === "Aufputz" && next.depth < 30) {
          next.depth = 30;
  ...
  const updateNotlicht = (key: keyof LiftConfig["notlicht"], value: string) => {
    onChange(prev => ({
      ...prev,
      notlicht: {
        ...prev.notlicht,
    ...
    <aside className="w-80 bg-sidebar border-r border-sidebar-border flex flex-col h-screen">
      ...
      <ScrollArea className="flex-1 overflow-y-auto">
        <div className="p-4 space-y-2 pb-8">
          ...
          <Collapsible open={openSections.bauweise} onOpenChange={() => toggleSection("bauweise")}>
            ...
            <CollapsibleContent className="pt-3 px-1 space-y-4">
              ...
              <label className="flex items-center gap-3 py-1 cursor-pointer">
                <Checkbox className={checkboxClass} checked={config.hinterwandkasten} onCheckedChange={c => update("hinterwandkasten", !!c)} />
          ...
          <Collapsible open={openSections.notlicht} onOpenChange={() => toggleSection("notlicht")}>
            <CollapsibleTrigger className="flex items-center justify-between w-full p-3 rounded-lg bg-sidebar-accent/50 hover:bg-sidebar-accent transition-colors">
              <div className="flex items-center gap-2">
                <CircleDot className="w-4 h-4 text-sidebar-primary" />
                <span className="text-sm font-medium text-sidebar-foreground">Notlichtschild ausfüllen</span>
              </div>
              <ChevronDown className={`w-4 h-4 text-sidebar-foreground/60 transition-transform ${openSections.notlicht ? 'rotate-180' : ''}`} />
            </CollapsibleTrigger>
            <CollapsibleContent className="pt-3 px-1 space-y-3">
              <div className="space-y-2">
                <Label className="text-xs text-sidebar-foreground/70 uppercase tracking-wide">Tragkraft / Personen</Label>
                <Input value={config.notlicht.tragkraft} placeholder="z. B. 1000 kg / 13 Personen" onChange={e => updateNotlicht("tragkraft", e.target.value)} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  <Label className="text-xs text-sidebar-foreground/70 uppercase tracking-wide">Baujahr</Label>
                  <Input value={config.notlicht.baujahr} placeholder="2024" onChange={e => updateNotlicht("baujahr", e.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs text-sidebar-foreground/70 uppercase tracking-wide">Umbaujahr</Label>
                  <Input value={config.notlicht.umbaujahr} placeholder="optional" onChange={e => updateNotlicht("umbaujahr", e.target.value)} />
                </div>
              </div>
              <div className="space-y-2">
                <Label className="text-xs text-sidebar-foreground/70 uppercase tracking-wide">Fabriknummer</Label>
                <Input value={config.notlicht.fabrNr} placeholder="Fabr.-Nr." onChange={e => updateNotlicht("fabrNr", e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label className="text-xs text-sidebar-foreground/70 uppercase tracking-wide">Hersteller</Label>
                <Input value={config.notlicht.hersteller} placeholder="Hersteller" onChange={e => updateNotlicht("hersteller", e.target.value)} />
              </div>
              <p className="text-[10px] text-sidebar-foreground/50 leading-relaxed">
                Die Daten werden direkt im Notlichtschild der 2D- und 3D-Ansicht aktualisiert und mittig ausgerichtet.
              </p>
            </CollapsibleContent>
          </Collapsible>
